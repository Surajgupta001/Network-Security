...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            if len(dataframe.columns) != number_of_columns:
                logging.error(
                    "Number of columns in the dataframe does not match the expected number."
                )
                return False
            return True
        except Exception as e:
    ...
    def initialize_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            logging.info("Data loaded successfully from train and test files.")

            error_message = ""
            # Validate the number of columns in train and test dataframes
            status = self.validate_number_of_columns(dataframe=train_dataframe)
            if not status:
                error_message = f"{error_message} Data validation failed: Number of columns in train data does not match the expected count."
            status = self.validate_number_of_columns(dataframe=test_dataframe)
            if not status:
                error_message = f"{error_message} Data validation failed: Number of columns in test data does not match the expected count."

            if len(error_message) > 0:
                raise Exception(error_message)

            # Lets check datadrift using KS test
            status = self.detect_dataset_drift(
                base_df=train_dataframe, current_df=test_dataframe
            )

            # Ensure output directory for validated CSVs exists
